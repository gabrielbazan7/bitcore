...
export class V8 {
  ...
  getMultisigContractConfirmationInfo(opts, cb) {
    const url = this.baseUrl + '/ethmultisig/confirmation/' + opts.multisigContractAddress;
    console.log('[v8.js.378:url:] CHECKING CONTRACT CONFIRMATION INFO', url);
    this.request
      .get(url, {})
      .then(multisigConfirmationInfo => {
        multisigConfirmationInfo = JSON.parse(multisigConfirmationInfo);
        return cb(null, multisigConfirmationInfo);
      })
      .catch(err => {
        return cb(err);
      });
