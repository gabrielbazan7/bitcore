...
*/
    ...
    router.post('/v1/ethmultisig/confirmation', (req, res) => {
      getServerWithAuth(req, res, async server => {
        try {
          const multisigContractConfirmationInfo = await server.getMultisigContractConfirmationInfo(req.body);
          res.json(multisigContractConfirmationInfo);
        } catch (err) {
          returnError(err, res, req);
        }
      });
