...
describe('Number format signing', function() {
  ...
  it('should refuse the prePublishRaw fallback on a proposal with no mutable tx data', function() {
    const txp = createTxp();
    const originalRaw = Utils.buildTx(txp).uncheckedSerialize();
    txp.prePublishRaw = originalRaw;
    (txp.outputs[0] as any).amount = '80000000000000000';
    Utils.buildTx(txp).uncheckedSerialize().should.not.deep.equal(originalRaw);

    Verifier.checkTxProposalSignature(credentials, txp).should.be.false;

    txp.deferNonce = true;
    Verifier.checkTxProposalSignature(credentials, txp).should.be.true;
  });

  it('should retain the existing prePublishRaw verification fallback for deferred proposals', function() {
    const txp = createTxp({ deferNonce: true });
    const originalRaw = Utils.buildTx(txp).uncheckedSerialize();
    txp.prePublishRaw = originalRaw;
    txp.nonce++;
    Utils.buildTx(txp).uncheckedSerialize().should.not.deep.equal(originalRaw);
    ...
    returned.prePublishRaw.should.deep.equal(originalRaw);
    Verifier.checkTxProposalSignature(credentials, returned).should.be.true;
    returned.prePublishRaw = '00';
    Verifier.checkTxProposalSignature(credentials, returned).should.be.false;
  });
});
